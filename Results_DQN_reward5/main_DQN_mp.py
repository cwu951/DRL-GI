...
import numpy as np
import pandas as pd
from SWMM_GR import SWMM_ENV_GI_utilization as SWMM_ENV # SWMM_ENV_GI_utilization instead of SWMM_ENV
...
os.environ["CUDA_VISIBLE_DEVICES"] = "-1" # Disable GPU visibility to force CPU usage
...
env_params={
        'orf':os.path.dirname(os.getcwd())+'/SWMM_GR/chaohu_noHC', # SWMM_GR instead of SWMM
        'orf_save':'chaohu_GI_RTC', # chaohu_GI_RTC instead of chaohu_RTC
...
agent_params={
    ...
    'action_table':pd.read_csv(os.path.dirname(os.getcwd())+'/SWMM_GR/action_table.csv').values[:,1:], # SWMM_GR instead of SWMM
}
...
def interact(i, ep):
    # Create a local copy of environment parameters for thread safety
    local_env_params = env_params.copy()
    local_env_params['process_id'] = i  # Assign unique process ID to prevent file conflicts
...
if Train:
    ...
    for epoch in range(model.params['training_step']):
        ...
        np.save('./Results/Train_GI.npy',history) # Train_GI instead of Train1
    
    # plot
    plt.figure()
    plt.plot(history['Episode_reward'])
    plt.savefig('./Results//Train_GI.tif') # # Train_GI instead of Train1

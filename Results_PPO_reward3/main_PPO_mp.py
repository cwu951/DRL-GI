...
agent_params = {
    ...
    'action_table': pd.read_csv(BASE_DIR + '/SWMM/action_table.csv').values[:, 1:],
}
...
if Train:
    ...
    for epoch in range(model.params['training_step']):
        ...
        np.save(RESULT_DIR + '/Train.npy', history)

    plt.figure()
    plt.plot(history['Episode_reward'])
    plt.savefig(RESULT_DIR + '/Train.tif')

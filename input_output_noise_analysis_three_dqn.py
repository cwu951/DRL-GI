...
# -*- coding: utf-8 -*-
"""
Input and output noise uncertainty analysis for three DQN models.
...
Noise rule used here:
    noisy_state_t = X1_t * state_t
    noisy_action_t = X2_t * action_t
where X1 and X2 are two independent noise samples and each element is sampled
independently from U(1 - delta, 1 + delta) for every dimension at every timestep.
"""
...
SCRIPT_TYPE = "input_output_noise"
MODEL_SPECS = [
    {"key": "reward5_gci", "label": "DQN reward5 GCI", "model_dir": "Results_DQN_reward5/model"},
    {"key": "reward3_base", "label": "DQN reward3 Base", "model_dir": "Results_DQN_reward3_o/model"},
    {"key": "reward3_gienv", "label": "DQN reward3 GIenv", "model_dir": "Results_DQN_reward3_train_GI_o/model"},
]
MODEL_DIR = None
...
class StandaloneSWMMEnv:
    ...
    def reset(self, rain_series: list[list[str]], rain_label: str) -> list[float]:
        ...
        # Keep temporary SWMM filenames short enough for Windows path limits.
        TEMP_DIR.mkdir(parents=True, exist_ok=True)
        short_label = rain_label.replace("design_", "d").replace("real_", "r")
        self.current_inp_path = TEMP_DIR / f"{self.params['orf_save']}_{short_label}.inp"
...
def run_single_simulation(
    ...
    state_seed, action_seed = seed_sequence.spawn(2)
    rng_state = np.random.default_rng(state_seed)
    rng_action = np.random.default_rng(action_seed)
    ...
    state_noise_history = []
    action_noise_history = []
    clean_state_history = []
    noisy_state_history = []
    action_index_history = []
    policy_action_history = []
    noisy_action_before_clip_history = []
    executed_action_after_clip_history = []

    done = False
    while not done:
        clean_state = np.asarray(observation, dtype=np.float64)
        state_noise_vector = sample_multiplicative_noise(rng_state, clean_state.size, delta)

        # State noise and action noise must be sampled independently.
        noisy_state = sanitize_state(clean_state * state_noise_vector)
        action_index, action_vector = agent.select_action(noisy_state.reshape(1, -1))
        action_noise_vector = sample_multiplicative_noise(rng_action, action_vector.size, delta)
        noisy_action_before_clip = action_vector * action_noise_vector
        executed_action = clip_action(noisy_action_before_clip)

        observation, reward, results, done = env.step(executed_action)

        clean_state_history.append(clean_state)
        state_noise_history.append(state_noise_vector)
        action_noise_history.append(action_noise_vector)
        noisy_state_history.append(noisy_state)
        action_index_history.append(action_index)
        policy_action_history.append(action_vector.copy())
        noisy_action_before_clip_history.append(noisy_action_before_clip.copy())
        executed_action_after_clip_history.append(executed_action.copy())
    ...
    return {
        ...
        "state_noise_history": np.asarray(state_noise_history, dtype=np.float64),
        "action_noise_history": np.asarray(action_noise_history, dtype=np.float64),
        ...
        "policy_action_history": np.asarray(policy_action_history, dtype=np.float64),
        "noisy_action_before_clip_history": np.asarray(
            noisy_action_before_clip_history, dtype=np.float64
        ),
        "executed_action_after_clip_history": np.asarray(
            executed_action_after_clip_history, dtype=np.float64
        ),
        "num_steps": len(state_noise_history),
...
def save_run_artifacts(run_result: dict) -> None:
    ...
    np.savez_compressed(
        ...
        state_noise_factors=run_result["state_noise_history"],
        action_noise_factors=run_result["action_noise_history"],
        ...
        policy_actions=run_result["policy_action_history"],
        noisy_actions_before_clip=run_result["noisy_action_before_clip_history"],
        executed_actions_after_clip=run_result["executed_action_after_clip_history"],
...
def main() -> None:
    ...
    print("\nInput + output noise analysis for three DQN models finished. Plotting is handled separately in the control notebook.")

...
# -*- coding: utf-8 -*-
"""
Output noise uncertainty analysis for three DQN models.
...
Noise rule used here:
    noisy_action_t = X_t * action_t
where each element of X_t is sampled independently from
U(1 - delta, 1 + delta) for every action dimension at every timestep.
"""
...
def build_environment() -> StandaloneSWMMEnv:
    env_params = {
        "orf": SWMM_INP,
        "orf_save": "chaohu_uncertainty_output",
        "parm": STATE_CONFIG,
...
def run_single_simulation(
    ...
    while not done:
        ...
        action_index, action_vector = agent.select_action(clean_state.reshape(1, -1))
        noise_vector = sample_multiplicative_noise(rng, action_vector.size, delta)

        # Noise is re-sampled independently for each action dimension at each timestep.
        noisy_action_before_clip = action_vector * noise_vector
...
def save_run_artifacts(run_result: dict) -> None:
    ...
    np.savez_compressed(
        ...
        action_noise_factors=run_result["noise_history"],
...
def main() -> None:
    ...
    print("\nOutput noise analysis for three DQN models finished. Plotting is handled separately in the control notebook.")
